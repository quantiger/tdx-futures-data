...
suffix_map = {'.CFF': 'CFF', '.DCE': 'DCE', '.CZC': 'CZC', '.SHF': 'SHF', '.INE': 'INE', '.GFE': 'GFE'}

futures = tq.get_stock_list('92', list_type=1)
print(f"主力合约: {len(futures)} 个")

os.makedirs('futures_daily', exist_ok=True)

collected = set(f.replace('.csv', '') for f in os.listdir('futures_daily') if f.endswith('.csv'))
...
for i, (code, orig) in enumerate(missing[:20]):
    print(f"  {i+1}. {orig} -> {code}")

total = len(missing)
for i, (code, orig) in enumerate(missing):
    if os.path.exists(f'futures_daily/{code}.csv'):
        continue
    ...
    merged.to_csv(f'futures_daily/{code}.csv', index=False)
    time.sleep(0.3)
